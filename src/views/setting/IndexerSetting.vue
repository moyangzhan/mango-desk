...
import { emptyDownloadState, emptyIndexerSetting } from '@/utils/functions'
type DownloadEvent =
  | {
    event: 'start'
    data: {
...
const indexerSetting = ref<IndexerSetting>(emptyIndexerSetting())
...
async function initStatusData() {
  ...
  const contentLaugnage = indexerSetting.value.file_content_language === 'en' ? t('common.english') : t('common.multilingual')
  if (!indexerSetting.value.is_private) {
    const platformInfo = modelPlatformList.value.find(p => p.name === activePlatform.value)
    if (platformInfo) {
      const aiModel = await invoke<AiModel>('load_model_by_type', { platform: activePlatform.value, oneType: 'vision' })
      if (aiModel && aiModel.name)
        imageParserDesc = `${platformInfo.title} => ${aiModel.title}`
      else
        imageParserDesc = t('common.disable')

      const audioModel = await invoke<AiModel>('load_model_by_type', { platform: activePlatform.value, oneType: 'asr' })
      if (audioModel && audioModel.name)
        audioParserDesc = `${platformInfo.title} => ${audioModel.title}`
      else
        audioParserDesc = t('common.disable')
    }
  }
  dataRef.value = [
    {
      ...
      children: [
        {
          ...
          status: indexerSetting.value.ignore_exts.join(', '),
          key: 'ignore-file-extensions',
        },
        {
          name: t('indexer.ignoreFiles'),
          status: indexerSetting.value.ignore_files.length === 0
            ? t('common.none')
            : indexerSetting.value.ignore_files.join(', '),
          key: 'ignore-files',
        },
        {
        ...
        },
        {
          ...
          status: indexerSetting.value.ignore_dirs.join(', '),
          key: 'ignore-folders',
        },
        {
        ...
        return h('span', {
          ...
          onClick: (e: PointerEvent) => {
            ...
            if (target.parentElement) {
              const trigger = target.parentElement.querySelector('.n-data-table-expand-trigger')
              if (trigger)
        ...
        return h('div', { class: 'flex flex-col' }, {
          default: () => [
            ...
            h(
              ...
              { default: () => `${t('common.multilingual')}: paraphrase-multilingual-MiniLM-L12-v2` },
            ),
          ],
        })
      } else {
...
async function doActivePlatformChanged(selectedName: string) {
  ...
  try {
    const res = await invoke('set_active_platform', { platformName: selectedName })
...
async function doPrivateModeChanged(enabled: boolean) {
  indexerSetting.value.is_private = enabled
  await updateIndexerSetting()
}

async function doContentLauguageChanged(lang: string) {
  indexerSetting.value.file_content_language = lang
  await updateIndexerSetting()
  ...
  try {
    ...
    onEvent.onmessage = (message) => {
      ...
      switch (message.event) {
        ...
        case 'progress':
          getDownloadState(message.data.downloadId).progress = message.data.progress
          break
        case 'finish': {
          getDownloadState(message.data.downloadId).progress = 100
          if (modelDownloadState.value.progress === 100 && tokenizerDownloadState.value.progress === 100) {
...
function getDownloadState(downloadId: string) {
  if (downloadId === modelDownloadState.value.downloadId)
    ...
    return tokenizerDownloadState.value
}

async function updateIndexerSetting() {
  try {
    const res = await invoke('update_indexer_setting', { indexerSetting: indexerSetting.value })
...
onMounted(async () => {
  ...
  try {
    indexerSetting.value = await invoke<IndexerSetting>('load_indexer_setting')
    activePlatform.value = await invoke<string>('load_active_platform')
    modelPlatformList.value = await invoke<ModelPlatform[]>('load_model_platforms')
    embeddingModelChanged.value = await invoke<boolean>('is_embedding_model_changed')
    let homePath = await invoke<string>('get_app_dir')
  ...
  <div>
    ...
    <NCard :title="t('indexer.fileContentLanguage')" class="mb-4 px-0" size="small" :bordered="true" hoverable>
      <div class="flex flex-col space-y-1 mb-4">
        <NRadioGroup :value="indexerSetting.file_content_language" @update:value="doContentLauguageChanged">
        ...
        <div
          v-if="indexerSetting.file_content_language === 'multilingual' && !embeddingModels.get('paraphrase-multilingual-MiniLM-L12-v2')"
          ...
          <div class="flex flex-col space-y-2 justify-start items-start">
            ...
            <NButton :type="downloadFailed ? 'error' : 'primary'" size="small" ghost tag="a" target="_blank"
              ...
              <span v-if="downloadFailed">{{ t('indexer.retryDownload') }}</span>
            </NButton>
          </div>
          <NProgress type="line" :percentage="modelDownloadState.progress" class="px-8"
            ...
            {{ `${t('model.embeddingModel')} ${modelDownloadState.progress.toFixed(2)}%` }}
          </NProgress>
          <NProgress type="line" :percentage="tokenizerDownloadState.progress" class="px-8"
            ...
            {{ `${t('model.tokenizer')} ${tokenizerDownloadState.progress.toFixed(2)}%` }}
          </NProgress>
          ...
          <NAlert type="warning" :show-icon="false">
            ...
            <div>
              1. {{ t('message.downloadLinks') }}
            </div>
            <div>
              • {{ t('common.model') }}:
              <n-text type="primary" class="cursor-pointer"
                @click="openUrl('https://huggingface.co/sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2/resolve/main/onnx/model.onnx')">
            ...
            <div>
              • {{ t('common.tokenizer') }}:
              <n-text type="primary" class="cursor-pointer"
                @click="openUrl('https://huggingface.co/sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2/resolve/main/tokenizer.json')">
                https://huggingface.co/sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2/resolve/main/tokenizer.json
              </n-text>
            </div>
            <div>
              2. {{ t('message.moveModelTip', { modelPath: modelPath }) }}
            </div>
          </NAlert>
        </div>
      </div>
    </NCard>
    <NCard :title="t('common.privacySetting')" class="mb-4" size="small" :bordered="true" hoverable>
      <div class="flex flex-col">
        <div class="flex flex-col space-y-1 mb-4">
          <NRadioGroup :value="indexerSetting.is_private" @update:value="doPrivateModeChanged">
            <NRadio :label="t('indexer.privateMode')" :value="true" />
            <NRadio :label="t('indexer.cloudMode')" :value="false" />
          </NRadioGroup>
          <NAlert :show-icon="false">
            <div v-if="indexerSetting.is_private">
              {{ t('indexer.privateModeDesc') }}
            </div>
            <div v-if="!indexerSetting.is_private">
              {{t('indexer.cloudModeDescDynamic', {
                modelPlatform: modelPlatformList.find(p => p.name
                  === activePlatform)?.title,
              })}}
            </div>
          </NAlert>
        </div>
        <NCard v-if="!indexerSetting.is_private" :title="t('indexer.cloudModeSetting')" size="small" :bordered="true">
